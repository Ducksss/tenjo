...
Name the customer categories, not partners. We have no partners, pilots or conversations with Nintendo, e+, Pia or anyone else, so never say or imply otherwise, and show no third-party logos. The demo drops borrow real names so they read instantly (Nintendo Switch 2, Taylor Swift), and each one says it's a demo drop and not affiliated. Use public facts as **evidence the market wants this**:
...
| 1:50–2:45 | **Nintendo Switch 2 · Pre-drop 2** (closed) → **Run draw** → Suiscan settlement tab                        | “Same engine, a console pre-drop. Anyone can press Run draw. First, Sui commits a random seed from `sui::random`, which nobody can predict or retry. Then one transaction settles everything: the shop is paid for its two consoles, and all four losers get their deposits back in that same transaction.” Show the balance changes. |
| 2:45–3:10 | Back on the drop page: evidence panel, then a loser’s code history                                         | “Your browser just re-ran the draw from the seed: same winners. We can’t rig this demo; Sui picked who lost. This fan starts the next pre-drop with two chances instead of one, and the counter lives on Sui, where nobody can edit it.”                                                                                              |
...
2. **Drops:** `npm run sui:demo-drop -- --live-drop` creates:
   ...
   - the open, real **Taylor Swift · Tokyo Night 2**.
