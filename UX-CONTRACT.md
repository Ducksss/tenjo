...
## Keeping extra chances with a real World ID

A real World ID gets a fresh code in every drop, so its losses only carry over through a passkey. When the site takes real World IDs and the browser supports passkeys, the entry card shows a periwinkle passkey step: one sentence on why, and a "Keep my extra chances with a passkey" choice that starts ticked. Unticked, entry is World ID alone. Ticked, the fan confirms with Face ID or a fingerprint before World ID: the first time it creates a Tenjō passkey (that creation is the entry's proof, so there is one prompt), later it signs this entry's World ID request. The step says how many losses the remembered passkey carries in the series, offers a passkey made on another device, and the receipt says the code comes from the passkey. A cancelled or interrupted passkey prompt enters nothing and offers "Confirm passkey" or "Enter with World ID alone". One passkey gets one entry per drop: a passkey already entered is refused before World ID is asked, so the proof stays unspent. One person gets one entry per drop whichever passkey they bring: World ID refuses their second proof, and Tenjō checks again against the drop's World ID code. On a paid drop the wallet only pays the deposit. The simulator keeps one code, so it never asks for a passkey.
