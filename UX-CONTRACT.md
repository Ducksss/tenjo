...
A real World ID gets a fresh code in every drop, so its losses only carry over through a wallet. When the site takes real World IDs, a free drop on Sui shows the same periwinkle wallet step, marked optional: one sentence on why, and the entry button stays enabled without it. With a wallet connected, the fan signs one free message before World ID (it names the drop, the wallet and the request), and the entry uses the wallet's code. On a paid drop the paying wallet does this without a signature. The step shows how many losses the connected wallet carries in the series, and the receipt says the code comes from the wallet. A refused signature enters nothing and says how to enter without the wallet. One wallet gets one entry per drop: a wallet already entered is refused before World ID is asked, so the proof stays unspent. One person gets one entry per drop whichever wallet they bring: World ID refuses their second proof, and Tenjō checks again against the drop's World ID code. The simulator keeps one code, so its entries never use a wallet's code.
